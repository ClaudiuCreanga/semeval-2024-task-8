...
class BertType(Enum):
    BERT = "bert"
    BERT_WITH_LAYER_SELECTION = "bert_with_layer_selection"
    HIERARCHICAL_BERT_WITH_POOLING = "hierarchical_bert_with_pooling"
...
class BERT(nn.Module):
    def __init__(
        ...
        self.bert_type = BertType.BERT
        if self.selected_layers is not None and len(self.selected_layers) > 0:
            self.bert_type = BertType.BERT_WITH_LAYER_SELECTION
            ...
            self.bert_type = BertType.HIERARCHICAL_BERT_WITH_POOLING

        input_size = None
        if self.bert_type == BertType.BERT_WITH_LAYER_SELECTION:
    ...
    def _do_reinit(self):
        self.bert.pooler.dense.weight.data.normal_(
        ...
        for n in range(self.reinit_last_n_layers):
            self.bert.encoder.layer[-(n + 1)].apply(self._init_weights_and_biases)
    ...
    def forward(self, input_ids, attention_mask):
        if self.bert_type == BertType.BERT:
            _, pooled_output = self.bert(
                input_ids=input_ids, attention_mask=attention_mask
            )
            output = self.drop_bert(pooled_output)
        elif self.bert_type == BertType.BERT_WITH_LAYER_SELECTION:
        ...
        elif self.bert_type == BertType.HIERARCHICAL_BERT_WITH_POOLING:
            ...
            for batch_sample_input_ids, batch_sample_attention_mask in zip(
                ...
                for chunk_input_ids, chunk_attention_mask in zip(
                    ...
                    _, chunk_pooled_output = self.bert(
            ...
            raise NotImplementedError(f"No such BERT type: {self.bert_type}")
    ...
    def freeze_transformer_layer(self):
        # TODO: If the reinit_last_n_layers is set,
        # the last n layers should be unfrozen ???
