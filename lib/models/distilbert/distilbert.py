...
class DistilBERTType(Enum):
    DISTILBERT = "distilbert"
    DISTILBERT_WITH_LAYER_SELECTION = "distilbert_with_layer_selection"
    HIERARCHICAL_BERT_WITH_POOLING = "hierarchical_distilbert_with_pooling"
...
class DistilBERT(nn.Module):
    def __init__(
        ...
        self.distilbert_type = DistilBERTType.DISTILBERT

        if self.selected_layers is not None and len(self.selected_layers) > 0:
            self.distilbert_type = DistilBERTType.DISTILBERT_WITH_LAYER_SELECTION
            ...
            self.distilbert_type = DistilBERTType.HIERARCHICAL_BERT_WITH_POOLING

        if self.distilbert_type == DistilBERTType.DISTILBERT_WITH_LAYER_SELECTION:
            ...
            if self.selected_layers_merge_strategy == MergeLayersStrategy.CONCATENATE:
                input_size = (
                    len(self.selected_layers) * self.distilbert.config.hidden_size
                )
            else:
        ...
        if self.device is not None:
            self.distilbert = self.distilbert.to(self.device)
    ...
    def _do_reinit(self):
        self.distilbert.pooler.dense.weight.data.normal_(
            mean=0.0, std=self.distilbert.config.initializer_range
        )
        self.distilbert.pooler.dense.bias.data.zero_()

        for param in self.distilbert.pooler.parameters():
            param.requires_grad = True

        for n in range(self.reinit_last_n_layers):
            self.distilbert.transformer.layer[-(n + 1)].apply(self._init_weights_and_biases)
    ...
    def forward(self, input_ids, attention_mask):
        if self.distilbert_type == DistilBERTType.DISTILBERT:
            _, pooled_output = self.distilbert(
                input_ids=input_ids, attention_mask=attention_mask
            )
            output = self.drop_bert(pooled_output)
        elif self.distilbert_type == DistilBERTType.DISTILBERT_WITH_LAYER_SELECTION:
        ...
        elif self.distilbert_type == DistilBERTType.HIERARCHICAL_BERT_WITH_POOLING:
            ...
            for batch_sample_input_ids, batch_sample_attention_mask in zip(
                ...
                for chunk_input_ids, chunk_attention_mask in zip(
                    ...
                    _, chunk_pooled_output = self.distilbert(
            ...
            raise NotImplementedError(f"No such BERT type: {self.distilbert_type}")
    ...
    def unfreeze_transformer_layer(self):
        if self.distilbert_type == DistilBERTType.DISTILBERT_WITH_LAYER_SELECTION:
            ...
            for selected_layer in self.selected_layers:
                for param in self.distilbert.transformer.layer[selected_layer].parameters():
                    param.requires_grad = True
    ...
    def get_hidden_layers_count(self) -> int:
        return len(self.distilbert.transformer.layer)
